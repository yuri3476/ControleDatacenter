...
function App() {
  ...
  const saveToFile = async () => {
    ...
    try {
      // Pega o workbook original do estado
      const { workbook } = excelData;

      // Cria a nova versão da planilha ATUAL com os dados do estado 'records'
      ...
      // **A MUDANÇA CRÍTICA:** Substitui a planilha antiga pela nova DENTRO do workbook existente
      workbook.Sheets[currentSheetName] = newWorksheet;

      // Converte o workbook MODIFICADO (com todas as planilhas) para um array de bytes
      const wbout = XLSX.write(workbook, { bookType: fileType, type: 'array' });
      
      // Escreve os dados no arquivo original
  ...
  const handleSaveAs = async () => {
      ...
      try {
          ...
          // A lógica aqui é a mesma do saveToFile: modificar o workbook atual e salvá-lo em um novo local.
          const { workbook } = excelData;
          const headers = ['Data', 'Hora', 'Nome', 'Item Verificado', 'Status', 'Observações'];
          const worksheetData = [headers, ...records.map(r => [r.Data, r.Hora, r.Nome, r['Item Verificado'], r.Status, r.Observações])];
          const newWorksheet = XLSX.utils.aoa_to_sheet(worksheetData);
